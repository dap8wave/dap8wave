...
function initializeStationPageLogic(stationPageWrapper, stationId) {
    ...
    tabLinks.forEach(link => {
        link.addEventListener('click', event => {
            ...
            if (targetId === 'da-list-content') {
                const password = prompt("Please enter the password to access the DA-List:");
                if (password === '22913322kK') {
    ...
    if (addDaForm) {
        addDaForm.addEventListener('submit', async event => {
            event.preventDefault();
            await addDoc(daListCollectionRef, { userId: addDaForm.userId.value, name: addDaForm.name.value, badgeId: addDaForm.badgeId.value, companyName: addDaForm.companyName.value, transporterId: addDaForm.transporterId.value });
            addDaForm.reset();
        });
    }
    ...
    if (addDriverToRosterForm) {
        addDriverToRosterForm.addEventListener('submit', async event => {
            ...
            await addDoc(rosterCollectionRef, {
                badgeId: badgeId, name: name, startTime: startTime, firmenname: firmenname,
                status: 'Awaiting', transporterId: `AT-${stationId.toUpperCase()}-${Math.floor(100 + Math.random() * 900)}`
            });
